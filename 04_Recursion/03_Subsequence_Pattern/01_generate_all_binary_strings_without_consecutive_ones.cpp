...
//! Total strings that can be created (valid + non-valid) is 2^n and O(n) time taken to copy(push_back) into result and O(n) for checking if valid or not at the end
//. T.C -> O(2^n * (n + n))
//. S.C -> O(n)
...
//! Total valid strings that will be generated will be less than 2^n and to copy (push_back) a string in ans array it takes O(n) T.C, as we need to iterate to all the indices, hence about TC is less than 2^n * n
