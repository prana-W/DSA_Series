...
//. S.C -> O(n + m), O(n) for returnign the answer and O(m) for the hashMap to store NGE (next greater element) of the nusm2. NOTE: O(m) is the only extra space used hhere apart from space taken for returnning the answer
